...
# Powerset of 4 (sample of it)

```{r sample-of-powerset4}
data("powerset04")
set.seed(1351)
dat <- sample(powerset04, size = 2000)
```

```{r correlation-04, cache=FALSE}
# Computing 
...
res <- cbind(a=rownames(all_cor04), as.data.frame(all_cor03))
res <- lapply(2:ncol(res), function(i) {
  data.frame(
    a    = res[,1],
    b    = colnames(res)[i],
...
# Simulation of size 5

```{r sample-of-5}
set.seed(71723)
dat <- replicate(2e3, matrix(sample.int(2, 25, TRUE)-1, nrow = 5, ncol = 5), simplify = FALSE)
```

```{r correlation-05, cache=FALSE}
# Computing 
all_raw04 <- similarity(dat, statistic = unlist(statistics))
all04 <- all_raw04
all04[!is.finite(all04)] <- NA
...
res <- cbind(a=rownames(all_cor04), as.data.frame(all_cor04))
res <- lapply(2:ncol(res), function(i) {
  data.frame(
    a    = res[,1],
    b    = colnames(res)[i],
